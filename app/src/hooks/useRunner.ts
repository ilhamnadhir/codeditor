...
export function useRunner(yTerminal: Y.Array<TerminalLine> | null): UseRunnerReturn {
    ...
    const run = useCallback(async (monacoLanguage: string, code: string, stdin = '', userName = 'You') => {
        if (running || !yTerminal)
            return;
        setRunning(true);
        runIdRef.current += 1;
        ...
        try {
            ...
            push(result.exitCode === 0 ? 'success' : 'stderr', `─── exit ${result.exitCode}  ·  ${result.language} ${result.version} ───`);
        }
        catch (err) {
