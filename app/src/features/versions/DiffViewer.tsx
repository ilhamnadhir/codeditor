...
/** Pure-JS unified diff using longest common subsequence */
function computeDiff(oldText: string, newText: string): DiffLine[] {
  ...
  // Collapse long unchanged runs to show only context around changes
...
function collapseContext(lines: DiffLine[], context = 3): DiffLine[] {
  ...
  changed.forEach(i => {
    for (let d = -context; d <= context; d++) {
      if (i + d >= 0 && i + d < lines.length) visible.add(i + d)
    }
  })
...
export default function DiffViewer({ snapshot, currentText }: Props) {
  ...
  return (
    <div>
      ...
      {diff.length === 0 ? (
        <div className="text-muted text-sm">No differences — content is identical.</div>
      ) : (
