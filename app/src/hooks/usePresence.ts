...
interface UsePresenceReturn {
  ...
  /** Update our own cursor position in awareness */
...
export function usePresence(
  ...
  // Set local user in awareness when user or provider changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const updateCollaborators = () => {
      ...
      states.forEach((state, clientId) => {
        ...
        // Estimate one-way latency via ping timestamp
  ...
  // Track edit count via Y.Doc observe
  useEffect(() => {
    if (!provider) return
    const doc = provider.doc

    const handleUpdate = (_update: Uint8Array, origin: unknown) => {
      // Only count local edits (origin === null for local)
  ...
  const updateSelection = useCallback(
    (sel: Collaborator['selection']) => {
      provider?.awareness.setLocalStateField('selection', sel)
    },
    [provider],
  )

  // Best latency estimate from all peers (min of all peer latencies)
  const bestLatency = collaborators.reduce<number | null>((best, c) => {
    if (c.latencyMs === null) return best
  ...
  return {
    collaborators,
    metrics: {
      activeUsers: collaborators.length + 1, // +1 for self
      editCount,
