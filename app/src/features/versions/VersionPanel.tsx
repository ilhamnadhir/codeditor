...
export default function VersionPanel({ roomId, activeFile, yText, currentContent, userId }: Props) {
    ...
    return (<div style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: '12px', height: '100%', overflow: 'auto' }}>
      ...
      <div>
        ...
        <div className="flex gap-2">
          <input className="input-field" style={{ height: 30, fontSize: 12, flex: 1 }} placeholder="Label (optional)" value={label} onChange={e => setLabel(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSave()}/>
          <button className="btn btn-primary btn-sm" onClick={handleSave} disabled={saving || !activeFile}>
      ...
      {showDiff && selectedSnap && (<div>
          <div className="flex items-center justify-between mb-2">
            <span style={{ fontSize: 12, fontWeight: 600 }}>Diff View</span>
            <button className="btn btn-ghost btn-sm" onClick={() => { setShowDiff(false); setSelectedSnap(null); }}>✕</button>
          </div>
      ...
      <div>
        ...
        {loading && (<div className="skeleton" style={{ height: 40, marginBottom: 4 }}/>)}

        {!loading && snapshots.length === 0 && (<div style={{ fontSize: 12, color: 'var(--text-muted)', padding: '12px 0' }}>
            No snapshots yet. Save one above.
          </div>)}

        {snapshots.map(snap => (<div key={snap.id} className="version-item" onClick={() => { setSelectedSnap(snap); setShowDiff(true); }} style={{
            ...
            <div className="version-item-actions">
              <button className="btn btn-ghost btn-sm" onClick={e => { e.stopPropagation(); handleRestore(snap); }} title="Restore">↩</button>
              <button className="btn btn-ghost btn-sm" onClick={e => { e.stopPropagation(); deleteSnapshot(snap.id); }} title="Delete">✕</button>
            </div>
          </div>))}
      </div>
    </div>);
