...
export default function RunPanel({ yTerminal, language, code, userName = 'You', running, onRun }: Props) {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="panel-header">
        ...
        {runtime && (<span className="badge" style={{ fontSize: 10, opacity: 0.7 }}>
            {runtime.version}
          </span>)}


        {!runtime && (<span className="badge badge-yellow" style={{ fontSize: 10 }}>
            Can't execute {LANGUAGE_LABELS[language] ?? language}
          </span>)}

        <div style={{ flex: 1 }}/>

        <button className="btn btn-ghost btn-sm btn-icon" onClick={() => setShowStdin(s => !s)} title="Toggle stdin input">⌨</button>

        <button className="btn btn-ghost btn-sm btn-icon" onClick={clear} title="Clear terminal">⊠</button>

        <button className={`btn btn-sm ${running ? 'btn-secondary' : 'btn-primary'}`} onClick={onRun} disabled={running || !runtime} id="run-in-panel-btn">
          {running
      ...
      {showStdin && (<div style={{ padding: '6px 12px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          <textarea value={stdin} onChange={e => setStdin(e.target.value)} placeholder="stdin (optional)…" style={{
      ...
      <div className="terminal-output">
        {lines.length === 0 && (<div style={{ color: 'var(--text-faint)', fontSize: 12 }}>
            {!runtime
        ...
        {lines.map((line, i) => (<div key={i} className={`terminal-line ${line.type}`}>
