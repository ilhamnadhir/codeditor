...
export default function AIPanel({ activeCode, activeFile, files, yFiles }: Props) {
    ...
    const getAllFiles = (): Record<string, string> | undefined => {
        if (!yFiles || files.length === 0)
            return undefined;
        const result: Record<string, string> = {};
        files.forEach(f => {
            const text = yFiles.get(f.name);
            if (text)
                result[f.name] = text.toString();
        });
        return result;
    };
    const handleAnalyze = () => {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="panel-tabs">
        {TABS.map(tab => (<button key={tab.id} className={`panel-tab ${activeTab === tab.id ? 'active' : ''}`} onClick={() => { setActiveTab(tab.id); clear(); }} title={tab.desc} id={`ai-tab-${tab.id}`}>
            {tab.icon} {tab.label}
          </button>))}
      </div>
      ...
      <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
        ...
        <div className="flex gap-2">
          <button className="btn btn-primary btn-sm w-full" onClick={handleAnalyze} disabled={loading || !activeCode.trim()} id={`ai-run-${activeTab}`}>
            {loading
            ? <><span className="spin">⟳</span> Analyzing…</>
            : <>{TABS.find(t => t.id === activeTab)?.icon} Analyze</>}
          </button>
          {response && (<button className="btn btn-ghost btn-sm btn-icon" onClick={clear} title="Clear">✕</button>)}
        </div>

        {!isConfigured && (<div style={{
                marginTop: 8,
                padding: '8px 10px',
      ...
      <div className="panel-body">
        ...
        {loading && !response && (<div className="ai-thinking">
            <span>Analyzing</span>
        ...
        {response ? (<div className="ai-response" dangerouslySetInnerHTML={{ __html: renderResponse(response) }}/>) : !loading && !error && (<div className="ai-empty">
