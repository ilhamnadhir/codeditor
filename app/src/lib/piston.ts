...
export async function executeCode(monacoLanguage: string, code: string, stdin = ''): Promise<ExecuteResult> {
    const rt = WANDBOX_COMPILERS[monacoLanguage];
    if (!rt) {
        throw new Error(`"${monacoLanguage}" isn't supported. Supported: ${Object.keys(WANDBOX_COMPILERS).join(', ')}`);
    }
    if (!code.trim()) {
