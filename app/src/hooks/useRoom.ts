...
export function useRoom(): UseRoomReturn {
  ...
  const joinRoom = useCallback(async (id: string, userId?: string): Promise<boolean> => {
    ...
    try {
      if (isSupabaseConfigured && userId) {
        ...
        if (fetchErr || !data) throw new Error('Room not found')

        // Add as editor if not already a member
        ...
        // No Supabase — just store locally
  ...
  const getUserRole = useCallback(async (roomId: string, userId: string): Promise<Role> => {
    if (!isSupabaseConfigured) return 'editor'
    const { data } = await supabase
      .from('room_members')
      .select('role')
      .eq('room_id', roomId)
